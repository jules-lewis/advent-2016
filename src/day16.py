...
the characters do not match (01 or 10), the next checksum character 
is a 0. This should produce a new string which is exactly half as long 
as the original. If the length of the checksum is even, repeat the 
...
#Expand the seed
def expand_seed(s, fill):
    while len(s) < fill:
        s2 = s[::-1]
        s2 = s2.replace('0', '#').replace('1', '0').replace('#', '1')
        s = s + '0' + s2
    return s[:fill]
...
s = '00101000101111010'
s = expand_seed(s, 272)
check_sum = get_checksum(s)
while (len(check_sum) % 2) == 0:
    check_sum = get_checksum(check_sum)
print(check_sum)

#Part 1
s = '00101000101111010'
s = expand_seed(s, 35651584)
check_sum = get_checksum(s)
while (len(check_sum) % 2) == 0:
    check_sum = get_checksum(check_sum)
print(check_sum)
